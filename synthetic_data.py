...
def main():
    ...
    data.args['characters_file_path'] = './my_characters.txt'
    print('The arguments :')
    print(data.args)
...
class OCRData(object):
    ...
    def saveAnnotation(self, content_index, points, image_index):
        for index, one_content in enumerate(content_index):
            ann_name = ''.join([str(image_index), '_', str(index), '.jpg'])
            ann_path = os.path.join(self.args['annotations_dir'], 'label.txt')
            #rectangle_points = utils.getOneLineRectanglePoints(points[index])
            with codecs.open(ann_path, 'a+', encoding='utf-8') as file:
                file.write(' '.join([ann_name, ''.join(one_content),'\n']))
